...
- **The internal channel is paired with its event types, and the channel itself is server-derived.** Two independent layers block a forged leg; both are now executed by tests. (1) EXECUTED: `createDomainRuntimeRegistry.accepts` refuses any delivery whose event type is a `handoff.*` on a channel other than `ORCHESTRATOR_HANDOFF`, and any `ORCHESTRATOR_HANDOFF` delivery whose event is not a `handoff.*`; channel and event type are declared independently in a contract, so without this pairing a customer-facing delivery naming a handoff event would resolve to a domain binding. Covered by `apps/worker/src/runtime/cross-domain/handoff-ingress.test.ts` (4 cases). (2) EXECUTED at the API boundary: a conversation turn whose body claims `source_channel: 'ORCHESTRATOR_HANDOFF'` with `event_type: 'handoff.marketing_to_sales'` is refused with HTTP 400 and starts no run and appends no message (`apps/api/src/routes/v1/conversations.test.ts`, "refuses a turn whose body claims the internal handoff channel, and derives the real one", which also asserts that an ordinary turn IS admitted and carries the conversation's own channel); the route's schema admits only the turn's own fields, and the admission path itself sets `source_channel` from the SERVER-RESOLVED conversation row (`apps/api/src/routes/v1/care-turn.ts`, `source_channel: conversation.channel`), never from the request.
...
- **Timeline.** The handoff's Customer 360 row is committed BY THE ADMISSION, in the same tenant transaction as the reservation, the target task and the ledger row (the `CareHandoffRepository` precedent): `admitCrossDomainHandoff` validates the row before the transaction opens — it must be keyed by the handoff idempotency key, positioned at the handoff instant, and carry `classification_authority: 'SERVER'` with the handoff's own classification — and it stores the caller-minted `handoff_id` as the ledger's primary key, stamping `evidence_reference` with that same value, so the ledger row, the admitted run's signal and the Customer 360 row cite ONE identity and the row can never reference an id that does not exist. A handoff that committed therefore always has its timeline row, and a replayed admission appends no second one (the stream's `(tenant_id, source_event_id)` deduplication key). The R15 projection projects a stored classification and its evidence reference ONLY when that server marker is present; otherwise it reports `SIGNAL` with a named `gap_reason`. Client deliveries carrying `classification`, `classification_authority` or `evidence_reference` are refused `CUSTOMER_EVENT_RESERVED_PAYLOAD_FIELD` at the single event port.
- **Journey wiring.**
- **`TC-E2E-001..009` is NOT an executable suite.** The cases exist as generated specifications, and `test:e2e` remains a declared-but-unimplemented task: no package implements it, no `apps/worker` end-to-end suite drives the three domains in one process, and no vitest config was added for it. What DOES exist is named per case rather than claimed as the suite: TC-E2E-001's lifecycle-and-single-timeline half runs against real PostgreSQL in the smoke (`5b`: marketing -> sales -> care admitted leg by leg, one lifecycle at version 3, three runs, three timeline rows in leg order, replayed final leg adds nothing); TC-E2E-002/003/007 and PILOT-01/02/04 have the offline pilot suites; TC-E2E-004(b,c)/005/006 have the RLS, adversarial and security suites. None of those is invoked AS the TC-E2E case, and no case drives the HTTP admission -> brokered handoff -> target run path end to end.
- **The DB-backed cross-domain smoke proves the LEDGER, not the HTTP path.** `scripts/p4-cross-domain-smoke.mjs` (9 cases, run by `pnpm test:integration`) drives the built `@agentos/database` package against real PostgreSQL as `agentos_app`: admission commits exactly one reservation, one target durable task and one ledger row; a replayed hop is answered `IN_FLIGHT` with the SAME run id and adds no task or row; the same key carrying different bytes is `CONFLICT`; a caller-asserted `FACT` is refused `HANDOFF_EVIDENCE_UNVERIFIED` with no write; the second hop advances the durable lifecycle and a repeated lifecycle version never admits; a hop addressed to a REAL customer of a second tenant is refused and writes nothing in either tenant (with a separate case for an unknown customer id); the whole marketing -> sales -> care journey is admitted leg by leg onto ONE lifecycle and ONE timeline; and the Customer 360 event appends once and reads back with its `SERVER` marker. It drives no API and no worker, so the HTTP admission → brokered handoff → target-run path is still unproven end to end.
- **The orchestrator's brokering has no live-database test.** It is covered by unit cases in `packages/core-engine/src/orchestrator/revenue-orchestrator.test.ts` (a completed run brokers exactly one handoff with a server-derived draft; a plan without an intent brokers nothing; a declared handoff with no broker bound fails `HANDOFF_BROKER_UNBOUND`; an unresolved admission parks the run at the next step cursor with its original `request_id`). Those cases prove the PARK, not a resume: no test yet re-claims a parked run and shows the broker re-invoked with the same handoff identity, and no test drives admission and brokering together against PostgreSQL.
...
- **The Care onboarding leg has no itinerary, and no requirement defines one.** A `sales → care` handoff admits its target run and the run classifies as the `care` leg, but no canonical CS-01 action is configured for that leg, so the planner REFUSES with `CARE_ONBOARDING_ITINERARY_UNBOUND` rather than returning an empty plan. The refusal is deliberate: an empty plan completes with zero steps and would report a journey leg as done that did nothing. Consequence of the gap is unchanged — no CS-01 work executes and no `care → retention` handoff is emitted, so the journey ends at Care until an itinerary is defined. What would close it is an OWNER DECISION, not engineering: the controlling SRS (`De_bai_…md`) contains no occurrence of the term at all, no FR maps to it, and none of the 8 canonical Care rows (`implement/05` rows 16-23) is an onboarding action. Outside this branch's own code, its tests and this ledger, the only matches in the repository are the roadmap's one-line lifecycle description (`implement/09` Sprint 9, "Marketing lead -> Sales consultation -> CSKH onboarding -> Reorder reminder") and three testcase-corpus entries that describe a newly onboarded TENANT in the SCR-001 empty-state fixture — a different subject entirely. Choosing which of them performs the leg — `skill.care.manage_case` (open a case for the new customer), a read of the customer's orders, or a new canonical row — is a product/architecture call that must not be guessed.
...
`P4 ENGINEERING MERGE READY: NO` — the branch is green on the local matrix, on the isolated PostgreSQL ring, in Docker and on GitHub CI, and the handoff contract, ledger, brokered admission, server-authoritative and non-duplicate timeline PROJECTION, the atomic timeline row, and the SCR-004 binding are delivered and tested. What is NOT delivered is named above rather than folded into "done": the `TC-E2E-001..009` acceptance suite and `test:e2e`, the Care onboarding itinerary that would reach the retention leg (the leg refuses today), the governed CS-02 retention enablement, and an authoritative Customer360 binding for the default Marketing composition. `FORMAL GATE P4 CLOSED: NO` — Gate P4 requires real runtime evidence (`handoff_packages`, `single_customer_context_proof`) from live adapters and upstream receipts, and no offline, mock or unit result closes it (`GOV-GATE-P4`, `implement/09 §1.1`).
