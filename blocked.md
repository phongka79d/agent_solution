...
- **The internal channel is paired with its event types, and the channel itself is server-derived.** Two independent facts block a forged leg, and they are stated separately because only the first is executed by a test. (1) EXECUTED: `createDomainRuntimeRegistry.accepts` refuses any delivery whose event type is a `handoff.*` on a channel other than `ORCHESTRATOR_HANDOFF`, and any `ORCHESTRATOR_HANDOFF` delivery whose event is not a `handoff.*`; channel and event type are declared independently in a contract, so without this pairing a customer-facing delivery naming a handoff event would resolve to a domain binding. Covered by `apps/worker/src/runtime/cross-domain/handoff-ingress.test.ts` (4 cases). (2) EXECUTED at the API boundary: a conversation turn whose body claims `source_channel: 'ORCHESTRATOR_HANDOFF'` with `event_type: 'handoff.marketing_to_sales'` is refused with HTTP 400 and starts no run and appends no message (`apps/api/src/routes/v1/conversations.test.ts`, "admits a turn on the CONVERSATION channel even when the body claims the internal handoff channel"); the route's schema admits only the turn's own fields, and the admission path itself sets `source_channel` from the SERVER-RESOLVED conversation row (`apps/api/src/routes/v1/care-turn.ts`, `source_channel: conversation.channel`), never from the request.
