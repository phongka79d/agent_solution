...
- **The internal channel is paired with its event types.** `createDomainRuntimeRegistry.accepts` refuses any delivery whose event type is a `handoff.*` on a channel other than `ORCHESTRATOR_HANDOFF`, and any `ORCHESTRATOR_HANDOFF` delivery whose event is not a `handoff.*`. Channel and event type are declared independently in a contract, so without this pairing a customer-facing delivery naming a handoff event would resolve to a domain binding and be routed as a brokered leg. Covered by `apps/worker/src/runtime/cross-domain/handoff-ingress.test.ts` (4 cases), which does not stand in for the missing TC-E2E suite: no API route is exercised.
