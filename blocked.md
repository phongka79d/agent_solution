...
- **Timeline.** An admitted handoff appends one `ext.lifecycle.handoff` event to `agentos.customer_events` keyed by the handoff idempotency key, with `classification_authority: 'SERVER'`. The R15 projection projects a stored classification and its evidence reference ONLY when that server marker is present; otherwise it reports `SIGNAL` with a named `gap_reason`. Client deliveries carrying `classification`, `classification_authority` or `evidence_reference` are refused `CUSTOMER_EVENT_RESERVED_PAYLOAD_FIELD` at the single event port.
...
- **The retention leg has no executable action.** `skill.care.analyze_churn_risk` is planned on `CS-02` and refuses at dispatch (`AUTHORITATIVE_SOURCE_UNAVAILABLE`/`SKILL_DISABLED`) because no authoritative `Customer360.AnalyticsLayer` is bound; `skill.care.issue_retention_offer` additionally needs an owner-approved floor (`ASM-003`/`ASM-004`, still unapproved). No churn score and no retention offer is fabricated in their place.
