...
- `pnpm lint` 9/9, `pnpm typecheck` 14/14, `pnpm test:unit` 14/14 (worker 610, core-engine 225, database 260), `pnpm test:contracts` 7/7, `pnpm test:adversarial` 7/7, `pnpm test:security` 7/7, `pnpm test:pilots` 6/6, `NODE_ENV=production pnpm build` 9/9, `git diff --check` clean. All of these were re-run unmasked on the final head, so a non-zero exit would have failed the run rather than being piped away.
- New P4 executable coverage: `packages/core-engine/src/contracts/cross-domain-handoff.test.ts` (18 tests — vocabulary, derived identity, every refusal code, malformed-input shapes) and `packages/core-engine/src/orchestrator/revenue-orchestrator.test.ts` (4 cases on the OUTCOME-stage brokering and its park path), `packages/database/src/repositories/cross-domain-handoffs.test.ts` (8 tests — ADMITTED/REPLAY/IN_FLIGHT/CONFLICT/RECONCILE_REQUIRED, unverified-FACT refusal, crash-recovery completion, and the three timeline-row refusals), `apps/worker/src/runtime/shared/cross-domain-handoff.test.ts` (6 tests — signal building, admission mapping, the Customer 360 row submitted WITH the admission, guard refusal propagation), plus the live-database ledger smoke below.
...
- DB-backed P1/P2 integration smokes against that ring with the in-process API-001 mock: `pnpm test:integration` 21/21 across the three suites (7 Care, 5 Sales, 9 P4 cross-domain ledger — the P4 suite added by this branch and wired into `test:integration` and into the CI database job through `P4_TENANT_IDS`).
