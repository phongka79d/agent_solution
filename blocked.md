...
| Durable restart / no duplicate handoff or effect | PARTIAL — the durable task/checkpoint/lease/effect machinery existed (P1B); no cross-domain hop existed | IMPLEMENTED for the handoff hop — the ledger admission reuses the reservation + durable-task pattern and is idempotent by construction |
...
| `TC-E2E-001..009` executable coverage | MISSING — the cases existed only as generated specifications; `test:e2e` is declared in the root manifest and `turbo.json` but implemented by no package | PARTIAL — P4-specific executable unit coverage exists (below); the `TC-E2E-001..009` suite and a DB-backed cross-domain smoke are still NOT implemented |
...
- **Timeline.** An admitted handoff appends one `ext.lifecycle.handoff` event to `agentos.customer_events` keyed by the handoff idempotency key, with `classification_authority: 'SERVER'`. The append is BEST-EFFORT AFTER the durable admission, not part of it: a failing append is swallowed (the handoff is already durable, and the dedupe key makes a later explicit append safe), so a handoff CAN exist with no timeline row and nothing retries it automatically — that gap is recorded below rather than claimed as guaranteed. The R15 projection projects a stored classification and its evidence reference ONLY when that server marker is present; otherwise it reports `SIGNAL` with a named `gap_reason`. Client deliveries carrying `classification`, `classification_authority` or `evidence_reference` are refused `CUSTOMER_EVENT_RESERVED_PAYLOAD_FIELD` at the single event port.
...
- DB-backed P1/P2 integration smokes against that ring with the in-process API-001 mock: `pnpm test:integration` 12/12 (7 Care, 5 Sales).
...
- **No DB-backed cross-domain smoke.** `scripts/p4-cross-domain-smoke.mjs` was not written and is not wired into `test:integration` or CI, so no live-database journey (admission → brokered handoff → target run) has been executed.
- **The handoff timeline row is not durable with the handoff.** The broker appends `ext.lifecycle.handoff` after the admission and swallows an append failure, so a durable handoff can exist without its Customer 360 row and nothing retries the append automatically. Making the append part of the admission transaction (or queueing an explicit reconciliation) is the fix; it is not done here.
- **The handoff path holds no DB-backed smoke.** `admitCrossDomainHandoff`, the orchestrator's brokering and the ledger's RLS behaviour are covered by unit suites with substituted repositories only; no live-database handoff has been admitted, so the ledger's `UNIQUE (tenant_id, idempotency_key)` and lifecycle-version race behaviour are unproven against PostgreSQL.
...
`P4 ENGINEERING MERGE READY: NO` — the branch is green on the local matrix, but the P4 acceptance suite (`TC-E2E-001..009`), a DB-backed cross-domain smoke, the retention action path and the default Marketing customer binding are not delivered, and the ledger above names them rather than folding them into "done". `FORMAL GATE P4 CLOSED: NO` — Gate P4 requires real runtime evidence (`handoff_packages`, `single_customer_context_proof`) and no offline, mock or unit result closes it (`GOV-GATE-P4`, `implement/09 §1.1`).
